...
def compute_gradient_saliency_maps(samples: torch.tensor,
    ...
    logits = (logits.gather(1, true_labels.view(-1, 1)).squeeze())
    #print(logits)
    
    logits.backward(torch.FloatTensor([1.0]*logits.shape[0]).to(device))
    #print(samples.grad.data)
    
    saliency, _ = torch.max(samples.grad.data.abs(), dim=1)
...
def main():  # pylint: disable=R0914, R0915
    ...
    for idx in range(all_real_saliency_maps.shape[0]):
        ...
        all_real_saliency_maps[idx] /= (all_real_saliency_maps[idx].max() + 1e-12)
