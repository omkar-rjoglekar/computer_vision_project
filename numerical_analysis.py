...
def get_soft_scores_and_true_labels(dataset, model):
    ...
    for batch_idx, (inputs, targets) in enumerate(dataloader):
        ...
        with torch.no_grad():
          ...
          # Move to gpu, run through model
          inputs, targets = inputs.to(device), targets.to(device)
          soft_scores = model(inputs)
          soft_scores=soft_scores.squeeze()
          #print(soft_scores, soft_scores.shape, type(soft_scores))

          all_first_soft_scores.append(soft_scores[0].item())
          all_second_soft_scores.append(soft_scores[1].item())
          gt_labels.append(targets.item())
